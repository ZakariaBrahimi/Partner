...
import { MetricCard, AnimatedNumber } from "@/mizaniya";
...
function trend(current: number, previous: number) {
  ...
  return { trend: `${Math.abs(change).toFixed(1)}%`, direction: (change >= 0 ? "up" : "down") as "up" | "down" };
}

export function PaymentMetrics({ current, previous }: { current: PaymentMetricsData; previous: PaymentMetricsData }) {
  const volumeTrend = trend(current.totalVolume, previous.totalVolume);
  const avgTrend = current.avgValue !== null && previous.avgValue !== null ? trend(current.avgValue, previous.avgValue) : { trend: undefined, direction: "up" as const };
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <MetricCard
        ...
        value={<AnimatedNumber value={current.successfulCount} />}
        comparison={current.successRate !== null ? `${current.successRate.toFixed(1)}% success rate` : "No data yet"}
      />
      <MetricCard
        icon={<XCircle className="size-[18px]" aria-hidden="true" />}
        title="Failed payments"
        value={<AnimatedNumber value={current.failedCount} />}
        comparison={current.totalCount > 0 ? `${((current.failedCount / current.totalCount) * 100).toFixed(1)}% of transactions` : "No data yet"}
      />
      <MetricCard
